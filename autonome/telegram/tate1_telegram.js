...
const telegramSession = process.env.TELEGRAM_SESSION_1;
const stringSession = new StringSession(telegramSession) || new StringSession("");
let client;
let isMyTurn = true; // Track if it's this account's turn
...
async function login() {
    try {
        ...
        await processChatHistory("chillguybitcoin", "@sigmaniac3")
        // Start AI chat rotation for the specified group
...
const joinNewGroupViaLink = async (inviteLink, telegramUsername) => {
    try {
        ...
        return {
            success: true,
            message: "Successfully joined group messages",
            groupName: groupName,
        };
    } catch (error) {
...
async function startAIChatRotation(groupName) {
    try {
        ...
        while (true) {
            try {
                if (isMyTurn) {
                    const botName = "@sigmaniac3";  // Changed from @goodbye000000 to @sigmaniac3
...
function manageTurns() {
    ...
    turnInterval = setInterval(() => {
        isMyTurn = !isMyTurn;
        console.log(`Turn status for @sigmaniac3: ${isMyTurn}`);  // Changed from @goodbye000000 to @sigmaniac3
    }, 5000);
